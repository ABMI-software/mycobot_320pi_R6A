...
+ Ajout : contrôle du GRIPPER PRO adaptatif (actions pro_gripper_*).
Le reste est identique à bridge_pi_simple.py.
"""
...
# NB : l'API gripper Pro (set_pro_gripper_*) est sur MyCobot320, pas MyCobot,
# et le 320 Pi communique à 1000000 bauds (pas 115200). D'où ce choix ici.
try:
    from pymycobot import MyCobot320 as MyCobot
    HAS_ROBOT = True
except ImportError:
    HAS_ROBOT = False
...
GRIPPER_ID = 14   # gripper_id par défaut du gripper Pro adaptatif
...
class BridgePiSimple:
    ...
    def __init__(self, host='0.0.0.0', port=5005, robot_port='/dev/ttyAMA0', baud=1000000):
        self.host = host
        self.port = port
        self.running = False
    ...
    def execute_json(self, cmd):
        ...
        try:
            ...
            # ===== AJOUT : Gripper PRO adaptatif (set_pro_gripper_*, id=14) =====
            # ⚠ Elephant impose >= 1.5 s entre deux commandes gripper : c'est au
            # client (PC Tour) d'espacer les envois.
            elif action == 'pro_gripper_open':
                self.mc.set_pro_gripper_open(cmd.get('gripper_id', GRIPPER_ID))
                return "OK: pro gripper ouvert"

            elif action == 'pro_gripper_close':
                self.mc.set_pro_gripper_close(cmd.get('gripper_id', GRIPPER_ID))
                return "OK: pro gripper fermé"

            elif action == 'pro_gripper_angle':
                a = max(0, min(100, int(cmd.get('angle', 100))))
                self.mc.set_pro_gripper_angle(cmd.get('gripper_id', GRIPPER_ID), a)
                return f"OK: pro gripper angle {a}"

            elif action == 'pro_gripper_calib':
                self.mc.set_pro_gripper_calibration(cmd.get('gripper_id', GRIPPER_ID))
                return "OK: pro gripper calibré (zéro)"

            elif action == 'get_pro_gripper_angle':
                a = self.mc.get_pro_gripper_angle(cmd.get('gripper_id', GRIPPER_ID))
                return f"PRO_GRIPPER_ANGLE: {a}"

            elif action == 'get_pro_gripper_status':
                st = self.mc.get_pro_gripper_status(cmd.get('gripper_id', GRIPPER_ID))
                return f"PRO_GRIPPER_STATUS: {st}"

            elif action == 'get_pro_gripper_torque':
                v = self.mc.get_pro_gripper_torque(cmd.get('gripper_id', GRIPPER_ID))
                return f"PRO_GRIPPER_TORQUE: {v}"

            elif action == 'get_pro_gripper_speed':
                v = self.mc.get_pro_gripper_default_speed(cmd.get('gripper_id', GRIPPER_ID))
                return f"PRO_GRIPPER_SPEED: {v}"

            elif action == 'get_pro_gripper_position':
                v = self.mc.get_pro_gripper_angle(cmd.get('gripper_id', GRIPPER_ID))
                return f"PRO_GRIPPER_POSITION: {v}"

            elif action == 'set_pro_gripper_speed':
                sp = max(1, min(100, int(cmd.get('speed', 50))))
                self.mc.set_pro_gripper_speed(cmd.get('gripper_id', GRIPPER_ID), sp)
                return f"OK: pro gripper speed {sp}"

            elif action == 'set_pro_gripper_torque':
                tq = max(100, min(300, int(cmd.get('torque', 150))))
                self.mc.set_pro_gripper_torque(cmd.get('gripper_id', GRIPPER_ID), tq)
                return f"OK: pro gripper torque {tq}"

            elif action == 'get_pro_gripper':
                # Lecture registre générique — pour sonder courant/force (adresse LCD).
                addr = int(cmd.get('address'))
                v = self.mc.get_pro_gripper(cmd.get('gripper_id', GRIPPER_ID), addr)
    ...
    def execute_raw(self, cmd):
        ...
        try:
            ...
            # AJOUT : gripper Pro en commande texte
            elif cmd_lower == 'pro_gripper_open':
                self.mc.set_pro_gripper_open(GRIPPER_ID)
                return "OK: pro gripper ouvert"

            elif cmd_lower == 'pro_gripper_close':
                self.mc.set_pro_gripper_close(GRIPPER_ID)
                return "OK: pro gripper fermé"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Bridge Pi Simple + Gripper Pro')
    ...
    print("=" * 50)
    print("🤖 Bridge Pi - Simple + Gripper Pro")
    print("=" * 50)
    print(f"📡 Réseau: {args.host}:{args.port}")
    print(f"🔌 Robot: {args.robot_port}")
    print("=" * 50)
    print("\nCommandes gripper Pro:")
    print("  {\"action\": \"pro_gripper_open\"}  /  pro_gripper_close")
    print("  {\"action\": \"pro_gripper_angle\", \"angle\": 40}")
    print("=" * 50 + "\n")
