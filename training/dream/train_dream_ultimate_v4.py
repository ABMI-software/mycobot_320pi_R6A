#!/usr/bin/env python3
"""DREAM training — ULTIMATE v4 (same proven recipe as v2 → 97.7).

Identical training recipe to train_dream_ultimate_v2.py (the run that reached
97.7): per-keypoint weighted loss, strong sim-to-real augmentation, cosine
annealing LR, early stopping. Only the DATASET default and OUTPUT tag change,
plus an OPTIONAL --pretrained flag to warm-start from the v2 weights.

What v4 targets:
  - New clean synthetic 50k (capsule self-collision filter, uniform angles,
    corrected camera intrinsics fov=1.15/fx≈494, top-cam 0.95 m):
    dream_data/synthetic_50k_ndds  (default --data)
  - Output tag: vgg_ultimate_v4_e{epochs}  (won't overwrite v2 checkpoints)

Per-keypoint loss weights (the "bonne weight" from v2):
  base=1.0, link1=1.0, link2=1.0, link3=1.0, link4=1.5, link5=1.5, link6=6.0

Usage:
  # Train from scratch on the new 50k (v2 recipe):
  python train_dream_ultimate_v4.py --epochs 50 --batch-size 8

  # Warm-start (finetune) from the vgg_real_ultimate_v2 weights:
  python train_dream_ultimate_v4.py \
      --pretrained checkpoints_dream/vgg_real_ultimate_v2_e15/best_network.pth \
      --epochs 30

  # Override dataset / weights:
  python train_dream_ultimate_v4.py -d <ndds_dir> --kp-weights 1,1,1,1,2,4,8

NOTE: requires the DREAM library at /tmp/DREAM (set up the env before running).
"""
...
# New clean synthetic 50k in NDDS format (default training data for v4).
DEFAULT_DATA = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "dream_data", "synthetic_50k_ndds",
)
    ...
    def patched_getitem(self, idx):
        if self.augment_data:
            ...
            strong_aug = albu.Compose([
                ...
                albu.Perspective(scale=(0.03, 0.07), p=0.3),
...
def train_weighted(args):
    ...
    # Optional warm-start from a pretrained checkpoint (e.g. ultimate_v2 weights)
    if args.pretrained:
...
def main():
    ...
    parser.add_argument("--data", "-d", default=DEFAULT_DATA,
                        help=f"NDDS data directory (default: {DEFAULT_DATA})")
    parser.add_argument("--pretrained", "-p", default=None,
                        help="Optional .pth to warm-start from "
                             "(e.g. checkpoints_dream/vgg_real_ultimate_v2_e15/best_network.pth)")
    ...
    if args.output is None:
        ...
        args.output = os.path.join(
            ...
            f"{args.arch}_ultimate_v4{tag}_e{args.epochs}"
        )
    train_weighted(args)
