...
def train_weighted(args):
    ...
    for epoch in range(args.epochs):
        ...
        if early_stopping.should_stop:
            print(f"\n🛑 Early stopping déclenché à l'epoch {epoch_num}")
            print(f"   Meilleur epoch : {early_stopping.best_epoch}")
            break
    ...
    if len(train_log["per_kp_val_losses"]) > 1:
        ...
        # Courbe train/val loss
        plt.figure(figsize=(8, 5))
        plt.plot(train_log["epochs"], train_log["losses"],
                marker='o', label='train_loss')
        plt.plot(train_log["epochs"], train_log["validation_losses"],
                marker='s', label='val_loss')
        best_epoch_idx = np.argmin(train_log["validation_losses"])
        plt.axvline(x=train_log["epochs"][best_epoch_idx],
                    color='red', linestyle='--', label=f'best epoch={best_epoch_idx+1}')
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title(f"w4={kp_weights[4]} | w5={kp_weights[5]} | w6={kp_weights[6]}")
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(args.output, "curve.png"))
        plt.close()
        print(f"  📊 Courbe sauvegardée → {args.output}/curve.png")    
...
def main():
    ...
    WEIGHTS_RANGE = [1.5, 3.0, 5.0, 7.0]
    
    for L4 in WEIGHTS_RANGE:
        for L5 in WEIGHTS_RANGE:
            for L6 in WEIGHTS_RANGE:
                args.kp_weights = f"1,1,1,1,{L4},{L5},{L6}"
                args.output = os.path.join(
                    SCRIPT_DIR, "checkpoints_dream",
                    f"{args.arch}_grid_w4{L4}_w5{L5}_w6{L6}_e{args.epochs}"
                )
                # Skip si déjà fait
                if os.path.exists(os.path.join(args.output, "best_network.pth")):
                    print(f"⏭️ Déjà fait → {args.output}")
                    continue
                print(f"\n{'='*60}")
                print(f"▶ link4={L4} | link5={L5} | link6={L6}")
                print(f"{'='*60}")
                train_weighted(args)  
