...
OXE_DATASET_CONFIGS = {
    ...
    ### Gazebo_to_LeRobot_Pipeline MyCobot 320 Pi sim sandbox (pipeline smoke test -- 2 scripted
    ### episodes, no real task; see ROS2_to_RLDS_Conversion_OpenVLA). State is
    ### already packed as a single 8-dim [xyz, quat, gripper] tensor (matching
    ### berkeley_autolab_ur5's pattern), so no EEF_state/gripper_state split
    ### is needed here.
    "gazebo_to_lerobot_mycobot": {
        "image_obs_keys": {"primary": "image", "secondary": None, "wrist": None},
        "depth_obs_keys": {"primary": None, "secondary": None, "wrist": None},
        "state_obs_keys": ["state"],
        "state_encoding": StateEncoding.POS_QUAT,
