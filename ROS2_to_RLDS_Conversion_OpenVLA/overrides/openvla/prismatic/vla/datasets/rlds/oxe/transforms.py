...
def gazebo_to_lerobot_mycobot_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """gazebo_to_lerobot_mycobot already stores state/action in this file's target
    layout (see ROS2_to_RLDS_Conversion_OpenVLA/extraction/extract_episodes.py
    and gazebo_to_lerobot_mycobot_dataset_builder.py: state = [xyz, quat, gripper],
    action = [xyz_delta, rpy_delta, gripper]) -- no field renaming or
    concatenation needed, unlike most OXE datasets in this file.

    The one real piece of work: the gripper channel is the raw
    gripper_controller URDF joint position in radians (URDF limit
    lower=-1.20, upper=0.10, rest=0.0), not yet in this file's "+1 = open,
    0 = closed" convention. Direction (which limit is open) comes from a
    visual check of the sim footage -- the gripper visibly opens as the
    joint moves toward its lower limit -- not an independent calibration
    of the physical/simulated gripper, so treat the sign as provisional
    if this dataset is ever used for more than a pipeline smoke test.
    """
    GRIPPER_LOWER, GRIPPER_UPPER = -1.20, 0.10  # URDF joint limits; lower = open, upper = closed

    def _normalize_gripper(raw):
        return (GRIPPER_UPPER - raw) / (GRIPPER_UPPER - GRIPPER_LOWER)

    state = trajectory["observation"]["state"]
    trajectory["observation"]["state"] = tf.cast(
        tf.concat([state[:, :7], _normalize_gripper(state[:, 7:8])], axis=-1), tf.float32
    )

    action = trajectory["action"]
    trajectory["action"] = tf.cast(
        tf.concat([action[:, :6], _normalize_gripper(action[:, 6:7])], axis=-1), tf.float32
    )
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "gazebo_to_lerobot_mycobot": gazebo_to_lerobot_mycobot_dataset_transform,
}
